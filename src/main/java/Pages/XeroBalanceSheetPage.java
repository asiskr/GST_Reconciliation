...
public class XeroBalanceSheetPage extends BaseClass {
	...
	public void fetchingGST() {
	    ...
	    Double totalValue = LAST_TABLE_DATA.size() > 3 && LAST_TABLE_DATA.get(3).containsKey("Total") 
	            ? LAST_TABLE_DATA.get(3).get("Total") 
	            : 0.0;
	    Double gstAsPerBalanceSheetValue = LAST_TABLE_DATA.size() > 4 && LAST_TABLE_DATA.get(4).containsKey("GST as per Balance sheet") 
	            ? LAST_TABLE_DATA.get(4).get("GST as per Balance sheet") 
	            : 0.0;

	    hm6.put("Total - GST as per balance sheet", totalValue - gstAsPerBalanceSheetValue);
